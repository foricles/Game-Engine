...
namespace kmu
{
	...
	inline float &mat::at(int i, int j) const
	{
		return oData[i * oN + j];
	}

	inline float &mat::at(int i, int j)
	{
		return oData[i * oN + j];
	}

	inline const mat & mat::mult(const mat & other)
	{
		mat res(oN, other.oM);
		if (oN == other.oM)
		{
	...
	inline int mat::getN() const
	{
		return oN;
	}

	inline int mat::getM() const
	{
		return oM;
	}

	inline const mat & mat::operator*=(const mat & other)
	{
		if (oN == other.oM)
		{
			mat res(oN, other.oM);
	...
	inline mat mat::operator *(const mat & other) const
	{
		if (oN == other.oM)
		{
			mat res(oN, other.oM);
	...
	inline std::string mat::toString() const
	{
		std::string res = "";
...
inline const kmu::mat4 & kmu::mat4::operator=(const mat4 & other)
{
	oN = other.oN;
	oM = other.oM;
	delete[]oData;
...
inline const kmu::vec4 kmu::mat4::operator*(const vec4 & right)
{
	kmu::vec4 ret;
